...
contract SingleMiningPoolV2 is OwnableUpgradeable {
    ...
    address public constant WETH = 0x8F8526dbfd6E38E3D8307702cA8469Bae6C56C15;
    // address public constant WETH = 0x70c1c53E991F31981d592C2d865383AC0d212225; // testnet
    // address public constant WETH = 0x5FbDB2315678afecb367f032d93F642f64180aa3; // local testnet
    
    // 10000 base to support the Fixed Deflation Token 
    mapping(uint256 => uint256) public  poolScale;
    ...
    function setPoolScale(uint256 _pid, uint256 _scale) external {

        require(msg.sender == timeLock, "SetPoolScale only timeLock allowed");
        poolScale[_pid] = _scale;
    }
    ...
    function setTimelock(address _timeLock) external {
        if(timeLock == address(0)) {
            require(msg.sender == owner(), "Init timelock only owner allowed");
            timeLock = _timeLock;
        } else {
            require(msg.sender == timeLock, "Update timelock only preTimeLock allowed");
            timeLock = _timeLock;
        }
    }
    ...
    function emergencyExitStrategy(uint256 _pid) external onlyOwner {
        ...
        require(address(pool.lpToken) != WETH, "WETH strategy cannot be address(0), pause in Strategy");
    ...
    function setStrategy(uint256 _pid, address _strategy) public {
        ...
        if(poolStrategy[_pid] == address(0)) {
            ...
            if(_strategy == address(0)) {
                require(address(pool.lpToken) != WETH, "Strategy for WETH cannot migrate to address(0)");
    ...
    function pendingJf(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = totalSupply(_pid);
        ...
        uint straPending = 0;
        if(poolStrategy[_pid] != address(0)) {
            straPending = IBaseStrategy(poolStrategy[_pid]).pendingJf(_user, user.amount);
        }
        return user.amount.mul(accJfPerShare).div(1e12).sub(user.rewardDebt).add(straPending);
        //return user.amount.mul(accJfPerShare).div(1e12).sub(user.rewardDebt).add(straPending).add(user.pending);
    }
    ...
    function _claimJf(uint256 _pid, address _account) private returns (uint256 _jfAmount){
        ...
        // if(user.amount > 0) {
            ...
            if(poolStrategy[_pid] != address(0)) {
                IBaseStrategy(poolStrategy[_pid]).harvest(msg.sender, user.amount);
            }
        // }
    }
    ...
    function totalSupply(uint256 _pid) public view returns (uint256 _totalSupply) {
        // totalDeposit
        if(poolStrategy[_pid] == address(0)) {
            PoolInfo storage pool = poolInfo[_pid];
            _totalSupply = (address(pool.lpToken) == WETH)? address(this).balance : pool.lpToken.balanceOf(address(this));
        } else {
            _totalSupply = IBaseStrategy(poolStrategy[_pid]).totalDeposit();
        }
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 lpSupply = totalSupply(_pid);// (address(pool.lpToken) == WETH)? address(this).balance : pool.lpToken.balanceOf(address(this));
        if (lpSupply == 0) {
    ...
    function deposit(uint256 _pid, uint256 _amount) public payable {
        ...
        if(_amount > 0) {
            bool hasStrategy =  poolStrategy[_pid] != address(0);
            address _receiveAddr = hasStrategy ? poolStrategy[_pid] : address(this);

            if(address(pool.lpToken) == WETH) {
                require(_amount == msg.value, "Msg.value not equal _amount");
                if(hasStrategy) { // has strategy, asset will be send to strategyAddress
                    IWETH(WETH).deposit{value: msg.value}();
                    pool.lpToken.safeTransfer(_receiveAddr, _amount );
                }
            }else {
                require(msg.value == 0, "Msg.value will be zero");
                safeTransferFrom(address(pool.lpToken), address(msg.sender), _receiveAddr, _amount );
            }
            if(hasStrategy) {
                IBaseStrategy(_receiveAddr).deposit(msg.sender, _amount, user.amount);
            }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "SinglePool:withdraw: not good");
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accJfPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0) {
            user.pending += pending;
        ...
        if(_amount > 0) {
            ...
            if(hasStrategy) {
                // notice:: Deflation Token not support Strategy
                require(poolScale[_pid] == 0, 'Deflation Token not support Strategy');
                // token will be transfered in Strategy
                IBaseStrategy(poolStrategy[_pid]).withdraw(msg.sender, _amount, user.amount);
            } else {
                if(address(pool.lpToken) == WETH) {
                    ...
                    if(poolScale[_pid] != 0) {
                        safeTransfer( address(pool.lpToken), address(msg.sender), _amount.mul(poolScale[_pid]).div(10000));
                    } else {
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        //todo assets may staked to Strategy
        bool hasStrategy =  poolStrategy[_pid] != address(0);
        if(hasStrategy) {
            // notice:: Deflation Token not support Strategy
            require(poolScale[_pid] == 0, 'Deflation Token not support Strategy');
            // token will be transfered in Strategy
            IBaseStrategy(poolStrategy[_pid]).withdraw(msg.sender, amount, amount);
        }else {
